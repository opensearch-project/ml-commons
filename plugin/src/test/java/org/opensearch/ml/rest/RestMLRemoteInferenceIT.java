...
public class RestMLRemoteInferenceIT extends MLCommonsRestTestCase {
    ...
    public void testPredictRemoteModelWithRecursiveParameter(
        String testCase,
        ...
        Response response = createConnector(this.getConnectorBodyByRecursiveParameter(testCase));
        Map responseMap = parseResponseToMap(response);
        String connectorId = (String) responseMap.get("connector_id");
    ...
    public void testPredictRemoteModelWithRecursiveParameterMissing() throws IOException, InterruptedException {
        testPredictRemoteModelWithRecursiveParameter("missing", null, (exception) -> {
            assertTrue(exception.getMessage().contains("Some parameter placeholder not filled in payload"));
        });
    }

    public void testPredictRemoteModelWithRecursiveParameterEnabled() throws IOException, InterruptedException {
        testPredictRemoteModelWithRecursiveParameter("enabled", null, (exception) -> {
            assertTrue(exception.getMessage().contains("Some parameter placeholder not filled in payload"));
        });
    }

    public void testPredictRemoteModelWithRecursiveParameterDisabled() throws IOException, InterruptedException {
        testPredictRemoteModelWithRecursiveParameter("disabled", (responseMap) -> {
            List responseList = (List) responseMap.get("inference_results");
            responseMap = (Map) responseList.get(0);
            responseList = (List) responseMap.get("output");
            responseMap = (Map) responseList.get(0);
    ...
    private String getConnectorBodyByRecursiveParameter(String testCase) {
        return switch (testCase) {
            ...
            case "disabled" -> "{\n"
                ...
                + "    \"temperature\": 0,\n"
                + "    \"model\": \"gpt-3.5-turbo-instruct\",\n"
                + "    \"recursive_parameter_enabled\": \"false\"\n"
                + "  },\n"
                + "  \"credential\": {\n"
                + "    \"openAI_key\": \""
                + this.OPENAI_KEY
