...
public class RestMLRemoteInferenceIT extends MLCommonsRestTestCase {
    ...
    @Ignore
    public void testOpenAICompletionModel() throws IOException, InterruptedException {
        String entity = "{\n"
        + "  \"name\": \"OpenAI Connector\",\n"
        + "  \"description\": \"The connector to public OpenAI model service for GPT 3.5\",\n"
        + "  \"version\": 1,\n"
    ...
    public void testOpenAIEditsModel() throws IOException, InterruptedException {
        ...
        responseList = (List) responseMap.get("choices");
        responseMap = (Map) responseList.get(0);
        assertTrue(((String) responseMap.get("text")).contains("What day of the week is it?"));
    }
    ...
    public void testCohereClassifyModel() throws IOException, InterruptedException {
        ...
        System.out.println(StringUtils.isJson(" { \"inputs\": [\"Confirm your email address\",\"hey i need u to send some $\"]}"));
        response = predictRemoteModel(modelId, predictInput);
        responseMap = parseResponseToMap(response);
        System.out.println(responseMap);
    }

    public void testSageMakerModel() throws IOException, InterruptedException {
        String entity = "{\n"
        ...
        Map responseMap = parseResponseToMap(response);
        String connectorId = (String) responseMap.get("connector_id");
        response = registerRemoteModel("sagemaker hugging face model", connectorId);
        responseMap = parseResponseToMap(response);
        String taskId = (String) responseMap.get("task_id");
        waitForTask(taskId, MLTaskState.COMPLETED);
        response = getTask(taskId);
        ...
        responseList = (List) responseMap.get("output");
        responseMap = (Map) responseList.get(0);
        assertFalse(((List) responseMap.get("data")).isEmpty());
    }

    public void testBedRockTextEmbeddingModel() throws IOException, InterruptedException {
        String entity = "{\n"
        ...
        Map responseMap = parseResponseToMap(response);
        String connectorId = (String) responseMap.get("connector_id");
        response = registerRemoteModel("bedrock embedding model", connectorId);
        responseMap = parseResponseToMap(response);
        String taskId = (String) responseMap.get("task_id");
        waitForTask(taskId, MLTaskState.COMPLETED);
        response = getTask(taskId);
