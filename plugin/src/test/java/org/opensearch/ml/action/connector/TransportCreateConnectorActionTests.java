...
public class TransportCreateConnectorActionTests extends OpenSearchTestCase {
    ...
    public void test_execute_withCustomConnectorId_usesCustomDocId() throws InterruptedException {
        ...
        doAnswer(invocation -> {
            ...
            listener.onResponse(indexResponse);
            return null;
        }).when(client).index(indexRequestCaptor.capture(), isA(ActionListener.class));

        action.doExecute(task, request, actionListener);

        verify(actionListener).onResponse(any(MLCreateConnectorResponse.class));
        assertEquals("my-openai-connector", indexRequestCaptor.getValue().id());
        // With a custom id, use CREATE op type so an existing id triggers a conflict instead of overwriting.
        assertEquals(DocWriteRequest.OpType.CREATE, indexRequestCaptor.getValue().opType());
    }

    public void test_execute_withoutConnectorId_usesAutoGeneratedDocId() throws InterruptedException {
        when(connectorAccessControlHelper.accessControlNotEnabled(any(User.class))).thenReturn(true);
        input.setAddAllBackendRoles(null);
        input.setBackendRoles(null);
        input.setConnectorId(null);
        ...
        // With no custom id, keep the default INDEX op type (auto-generated id cannot conflict).
    ...
    public void test_execute_withCustomConnectorId_alreadyExists_fails() throws InterruptedException {
        ...
        // Simulate a document id conflict from the index call (CREATE op type on an existing id).
        ...
        // The SDK wraps the version conflict into a 409 CONFLICT status exception surfaced to the user.
        assertTrue(argumentCaptor.getValue() instanceof OpenSearchStatusException);
        assertEquals(RestStatus.CONFLICT, ((OpenSearchStatusException) argumentCaptor.getValue()).status());
    }
