...
public class MLModelManagerTests extends OpenSearchTestCase {
    ...
    public void testRegisterMLModel_IndexModelChunkFailure() throws IOException {
        ...
        verify(modelHelper).downloadAndSplit(any(), any(), any(), any(), any(), any(), FunctionName.TEXT_EMBEDDING, any());
    }
    ...
    public void testRegisterMLModel_DownloadModelFileFailure() {
        ...
        verify(mlIndicesHandler).initModelIndexIfAbsent(any());
        verify(client).index(any(), any());
        verify(modelHelper).downloadAndSplit(eq(modelFormat), eq(modelId), eq(modelName), eq(version), eq(url), any(), FunctionName.TEXT_EMBEDDING, any());
    }
    ...
    private void setUpMock_DownloadModelFileFailure() {
        doAnswer(invocation -> {
            ...
            return null;
        }).when(modelHelper).downloadAndSplit(any(), any(), any(), any(), any(), any(), FunctionName.TEXT_EMBEDDING, any());
    }
