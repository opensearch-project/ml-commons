...
public class MLModelManagerTests extends OpenSearchTestCase {
    ...
    public void testRegisterMLRemoteModel_withCustomModelId_usesCustomDocId() throws PrivilegedActionException, IOException {
        ActionListener<MLRegisterModelResponse> listener = mock(ActionListener.class);
        doNothing().when(mlTaskManager).checkLimitAndAddRunningTask(any(), any());
        when(mlCircuitBreakerService.checkOpenCB()).thenReturn(null);
        when(threadPool.executor(REGISTER_THREAD_POOL)).thenReturn(taskExecutorService);
        when(modelHelper.downloadPrebuiltModelMetaList(any(), any())).thenReturn(Collections.singletonList("demo"));
        when(modelHelper.isModelAllowed(any(), any())).thenReturn(true);
        // Non-hidden remote model with a user-specified id.
        MLRegisterModelInput pretrainedInput = mockRemoteModelInput(false).toBuilder().modelId("my-custom-model").build();
        MLTask pretrainedTask = MLTask.builder().taskId("pretrained").modelId("pretrained").functionName(FunctionName.REMOTE).build();
        mock_MLIndicesHandler_initModelIndex(mlIndicesHandler, true);
        ...
        when(indexResponse.getId()).thenReturn("my-custom-model");
        modelManager.registerMLRemoteModel(sdkClient, pretrainedInput, pretrainedTask, listener);

        assertEquals("my-custom-model", indexRequestCaptor.getValue().id());
        // Custom id uses CREATE op type so an existing id triggers a conflict instead of overwriting.
        assertEquals(DocWriteRequest.OpType.CREATE, indexRequestCaptor.getValue().opType());
    }

    public void testRegisterMLRemoteModel_withoutModelId_usesAutoGeneratedDocId() throws PrivilegedActionException, IOException {
        ActionListener<MLRegisterModelResponse> listener = mock(ActionListener.class);
        doNothing().when(mlTaskManager).checkLimitAndAddRunningTask(any(), any());
        when(mlCircuitBreakerService.checkOpenCB()).thenReturn(null);
        when(threadPool.executor(REGISTER_THREAD_POOL)).thenReturn(taskExecutorService);
        when(modelHelper.downloadPrebuiltModelMetaList(any(), any())).thenReturn(Collections.singletonList("demo"));
        when(modelHelper.isModelAllowed(any(), any())).thenReturn(true);
        // Non-hidden remote model without a user-specified id.
        MLRegisterModelInput pretrainedInput = mockRemoteModelInput(false);
        MLTask pretrainedTask = MLTask.builder().taskId("pretrained").modelId("pretrained").functionName(FunctionName.REMOTE).build();
        mock_MLIndicesHandler_initModelIndex(mlIndicesHandler, true);
        ...
        // No custom id: OpenSearch auto-generates the id (no id set on the request) with default INDEX op type.
