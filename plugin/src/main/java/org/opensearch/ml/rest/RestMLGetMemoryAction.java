...
import static org.opensearch.ml.common.memorycontainer.MemoryContainerConstants.BASE_MEMORY_PATH;
...
public class RestMLGetMemoryAction extends BaseRestHandler {
        ...
        return ImmutableList
            .of(
                new Route(
                    ...
                    String.format(Locale.ROOT, "%s/{%s}/{%s}", BASE_MEMORY_PATH, PARAMETER_MEMORY_CONTAINER_ID, PARAMETER_MEMORY_ID)
                )
            );
    }
    ...
    public RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        MLMemoryGetRequest mlMemoryGetRequest = getRequest(request);
        return channel -> client.execute(MLMemoryGetAction.INSTANCE, mlMemoryGetRequest, new RestToXContentListener<>(channel));
    }
     ...
     */
    @VisibleForTesting
    MLMemoryGetRequest getRequest(RestRequest request) throws IOException {
        ...
        return new MLMemoryGetRequest(memoryContainerId, tenantId, memoryId);
    }
}
