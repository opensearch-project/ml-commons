    ...
    protected void doExecute(Task task, ActionRequest request, ActionListener<MLGetMemoryResponse> actionListener) {
        ...
        memoryContainerHelper.getMemoryContainer(memoryContainerId, ActionListener.wrap(container -> {
            ...
            try (ThreadContext.StoredContext context = client.threadPool().getThreadContext().stashContext()) {
                client.get(getMemoryRequest, ActionListener.wrap(
                    getResponse -> processResponse(getResponse, memoryId, memoryContainerId, actionListener),
                    exception -> {
        ...
        try {
            if (getResponse.isExists()) {
                try (
                    ...
                    MLGetMemoryResponse response = MLGetMemoryResponse.builder()
                        .mlMemory(mlMemory)
                        .build();
