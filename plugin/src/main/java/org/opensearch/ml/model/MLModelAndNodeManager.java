...
//        }
	...
	private void doAutoReLoadModel() {
		...
		if (allNodeIds.length != eligibleNodeIds.length) {  // 说明有挂掉的节点
			needReLoadModelNodeIds = Arrays.stream(allNodeIds)
					.filter(m -> !List.of(eligibleNodeIds).contains(m)).toArray(String[]::new);
		}
		...
		for (String needReLoadModelNodeId : needReLoadModelNodeIds) {
			...
			if (MLNodeUtils.isMLNode(node)) {
				String[] modelIds;
				try {
					modelIds = getAllLoadedModelIdsByNode(needReLoadModelNodeId);
				} catch (Exception e) {
					log.error("Failed to get all loaded model ids from node id {}, the reason is: {}",
							needReLoadModelNodeId, e);
	...
	private String[] getAllLoadedModelIdsByNode(String nodeId) {
			...
			client.search(searchRequest, ActionListener.wrap(response -> {
				...
				for (SearchHit hit : hits) {
					try (XContentParser parser = createXContentParserFromRegistry(xContentRegistry,
						...
						if (workerNode.contains(nodeId) && mlTaskType == MLTaskType.LOAD_MODEL
								&& mlTaskState == MLTaskState.COMPLETED) {
							result.add(mlTask.getModelId());
						}
					} catch (Exception e) {
				...
				if (e instanceof IndexNotFoundException) {
					...
					throw new IndexNotFoundException(ML_TASK_INDEX, e);
				} else {
					log.error("Resource from Index {} not found, the reason is: {}", ML_TASK_INDEX, e);
					throw new MLResourceNotFoundException(e);
				}
			}));
		} catch (Exception e) {
			log.error("Failed to get ML model from node {}, the reason is: {}", nodeId, e);
			throw new RuntimeException(e);
		}
