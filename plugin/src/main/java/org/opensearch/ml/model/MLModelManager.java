...
public class MLModelManager {
	...
	public MLModelManager(
		...
		clusterService.getClusterSettings()
				.addSettingsUpdateConsumer(ML_COMMONS_MAX_MODELS_PER_NODE, it -> maxModelPerNode = it);

		maxUploadTasksPerNode = ML_COMMONS_MAX_UPLOAD_TASKS_PER_NODE.get(settings);
		clusterService
				.getClusterSettings()
				.addSettingsUpdateConsumer(ML_COMMONS_MAX_UPLOAD_TASKS_PER_NODE,
						it -> maxUploadTasksPerNode = it);

		enableAutoReLoadModel = ML_COMMONS_MODEL_AUTO_RELOAD_ENABLE.get(settings);
		clusterService.getClusterSettings()
				.addSettingsUpdateConsumer(ML_COMMONS_MODEL_AUTO_RELOAD_ENABLE,
						it -> enableAutoReLoadModel = it);
	...
	public void uploadMLModel(MLUploadInput uploadInput, MLTask mlTask) {
		...
		try (ThreadContext.StoredContext context = client.threadPool().getThreadContext()
			...
			if (errorMsg != null) {
				mlTaskManager.updateMLTaskDirectly(mlTask.getTaskId(),
						ImmutableMap.of(STATE_FIELD, FAILED, ERROR_FIELD, errorMsg));
			...
			mlIndicesHandler.initModelIndexIfAbsent(ActionListener.wrap(res -> {
				...
				indexModelMetaRequest.source(
						mlModelMeta.toXContent(XContentBuilder.builder(JSON.xContent()), EMPTY_PARAMS));
				...
				ActionListener<IndexResponse> listener = ActionListener.wrap(modelMetaRes -> {
					...
					modelHelper.downloadAndSplit(modelId, modelName, version, uploadInput.getUrl(),
							ActionListener.wrap(result -> {
								...
								for (String name : chunkFiles) {
									...
									indexRequest.source(
											mlModel.toXContent(XContentBuilder.builder(JSON.xContent()), EMPTY_PARAMS));
									indexRequest.setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE);
									client.index(indexRequest, ActionListener.wrap(r -> {
										uploaded.getAndIncrement();
										if (uploaded.get() == chunkFiles.size()) {
											updateModelUploadStateAsDone(uploadInput, taskId, modelId, modelSizeInBytes,
													chunkFiles, hashValue);
										} else {
	...
	private <T> ThreadedActionListener<T> threadedActionListener(String threadPoolName,
	...
	private void updateModelUploadStateAsDone(
		...
		updateModel(modelId, updatedFields, ActionListener.wrap(updateResponse -> {
			mlTaskManager.updateMLTask(taskId,
					ImmutableMap.of(STATE_FIELD, COMPLETED, MODEL_ID_FIELD, modelId), TIMEOUT_IN_MILLIS,
					true);
	...
	private void loadModelAfterUploading(MLUploadInput uploadInput, String modelId) {
		...
		log.debug("start loading model after uploading {} on nodes: {}", modelId,
		...
		ActionListener<LoadModelResponse> listener = ActionListener
				.wrap(r -> log.debug("model loaded, response {}", r),
						e -> log.error("Failed to load model", e));
		client.execute(MLLoadModelAction.INSTANCE, request, listener);
	}

	private void deleteModel(String modelId) {
		DeleteRequest deleteRequest = new DeleteRequest();
		deleteRequest.index(ML_MODEL_INDEX).id(modelId)
	...
	private void handleException(FunctionName functionName, String taskId, Exception e) {
		mlStats.createCounterStatIfAbsent(functionName, UPLOAD,
				MLActionLevelStat.ML_ACTION_FAILURE_COUNT).increment();
		Map<String, Object> updated = ImmutableMap.of(ERROR_FIELD, ExceptionUtils.getStackTrace(e),
				STATE_FIELD, FAILED);
	 ...
	 */
	public void loadModel(String modelId, String modelContentHash, FunctionName functionName,
			ActionListener<String> listener) {
		mlStats.createCounterStatIfAbsent(functionName, ActionName.LOAD, ML_ACTION_REQUEST_COUNT)
				.increment();
		...
		try (ThreadContext.StoredContext context = client.threadPool().getThreadContext()
			...
			this.getModel(modelId,
					threadedActionListener(LOAD_THREAD_POOL, ActionListener.wrap(mlModel -> {
						if (mlModel.getAlgorithm()
								!= FunctionName.TEXT_EMBEDDING) {// load model trained by built-in algorithm like kmeans
	...
	private void handleLoadModelException(String modelId, FunctionName functionName,
			ActionListener<String> listener, Exception e) {
		mlStats.createCounterStatIfAbsent(functionName, ActionName.LOAD,
				MLActionLevelStat.ML_ACTION_FAILURE_COUNT).increment();
		removeModel(modelId);
	 ...
	 * @param listener action listener
	 */
	public void getModel(String modelId, String[] includes, String[] excludes,
	...
	private void retrieveModelChunks(MLModel mlModelMeta, ActionListener<File> listener)
		...
		for (int i = 0; i < totalChunks; i++) {
			...
			int currentChunk = i;
			this.getModel(modelChunkId,
					threadedActionListener(LOAD_THREAD_POOL, ActionListener.wrap(model -> {
						...
						if (retrievedChunks.get() == totalChunks - 1) {
							listener.onFailure(
									new MLResourceNotFoundException("Fail to find model chunk " + modelChunkId));
						}
					})));
		}
	}
		...
		if (modelIds != null && modelIds.length > 0) {
			...
			for (String modelId : modelIds) {
				if (modelCacheHelper.isModelLoaded(modelId)) {
					...
					mlStats
							.createCounterStatIfAbsent(getModelFunctionName(modelId), ActionName.UNLOAD,
			...
			for (String modelId : getLocalLoadedModels()) {
				modelUnloadStatus.put(modelId, UNLOADED);
				mlStats.getStat(MLNodeLevelStat.ML_NODE_TOTAL_MODEL_COUNT).decrement();
				mlStats.createCounterStatIfAbsent(getModelFunctionName(modelId), ActionName.UNLOAD,
						ML_ACTION_REQUEST_COUNT).increment();
				removeModel(modelId);
			}
		}
	...
	public void autoReLoadModel() {
		...
		Retryer<Void> retryer = RetryerBuilder.<Void>newBuilder()
				.retryIfExceptionOfType(Exception.class) // 发生异常则重试
