...
public class MLCommonsClusterEventListener implements ClusterStateListener {
    ...
    public void clusterChanged(ClusterChangedEvent event) {
        ...
        if (delta.removed()) {
            ...
            List<String> addedNodes = delta.addedNodes().stream().map(DiscoveryNode::getId).collect(Collectors.toList());
            mlModelAutoReDeployer.buildAutoReloadArrangement(addedNodes, state.getNodes().getClusterManagerNodeId());
        }
    }
}
