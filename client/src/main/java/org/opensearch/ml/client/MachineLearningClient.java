...
public interface MachineLearningClient {
     ...
     * @return the result future
     */
    default ActionFuture<MLExecuteTaskResponse> executeAlgorithm(FunctionName name, Input input) {
        PlainActionFuture<MLExecuteTaskResponse> actionFuture = PlainActionFuture.newFuture();
        executeAlgorithm(name, input, actionFuture);
        return actionFuture;
    }
     ...
     * @param listener a listener to be notified of the result
     */
    void executeAlgorithm(FunctionName name, Input metricsCorrelationInput, ActionListener<MLExecuteTaskResponse> listener);
}
