...
public class ConversationalMemoryHandlerITTests extends OpenSearchIntegTestCase {
    ...
    public void testCanGetInteractionsBackOut() {
        ...
        LatchedActionListener<List<ConversationMeta>> finishAndAssert = new LatchedActionListener<>(ActionListener.wrap(conversations -> {
            ...
            assert (interactions.get(0).getId().equals(id1));
            assert (interactions.get(1).getId().equals(id2));
    ...
    public void testDifferentUsers_DifferentConversations() {
        try (ThreadContext.StoredContext threadContext = client.threadPool().getThreadContext().stashContext()) {
            ...
            inter3.whenComplete(inters -> {
                ...
                assert (inters.get(1).getId().equals(iid5.result()));
            ...
            inter1.whenComplete(inters -> {
                assert (inters.size() == 2);
                assert (inters.get(0).getId().equals(iid1.result()));
                assert (inters.get(1).getId().equals(iid2.result()));
