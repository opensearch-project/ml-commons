...
public class MLCreateControllerRequestTest {
    ...
    public void setUp() throws Exception {
        ...
        controllerInput = MLController.builder().modelId("testModelId").userRateLimiter(new HashMap<>() {
            {
                put("testUser", rateLimiter);
            }
        }).build();
        request = MLCreateControllerRequest.builder().controllerInput(controllerInput).build();
    }
    ...
    public void writeToSuccess() throws IOException {
        ...
        MLCreateControllerRequest parsedRequest = new MLCreateControllerRequest(bytesStreamOutput.bytes().streamInput());
        ...
        assertEquals("1", parsedRequest.getControllerInput().getUserRateLimiter().get("testUser").getLimit());
        assertEquals(TimeUnit.MILLISECONDS, parsedRequest.getControllerInput().getUserRateLimiter().get("testUser").getUnit());
    }
    ...
    public void validateWithNullMLModelID() {
        ...
        MLCreateControllerRequest request = MLCreateControllerRequest.builder().controllerInput(controllerInput).build();
