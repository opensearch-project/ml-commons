        ...
        initConversationMetaIndexIfAbsent(ActionListener.wrap(r -> {
            if(r) {
                ...
                IndexRequest request = Requests.indexRequest(indexName).source(
                    ConversationalIndexConstants.META_CREATED_FIELD, Instant.now(),
                    ConversationalIndexConstants.META_ENDED_FIELD, Instant.now(),
                    ConversationalIndexConstants.META_LENGTH_FIELD, 0,
                    ConversationalIndexConstants.META_NAME_FIELD, name,
                    ConversationalIndexConstants.USER_FIELD, userstr==null ? null : User.parse(userstr).getName()
                );
                try (ThreadContext.StoredContext threadContext = client.threadPool().getThreadContext().newStoredContext(true)) {
