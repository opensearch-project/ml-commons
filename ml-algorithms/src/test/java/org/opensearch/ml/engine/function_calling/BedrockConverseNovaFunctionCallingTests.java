...
public class BedrockConverseNovaFunctionCallingTests {
    ...
    @Before
    public void setUp() {
        functionCalling = FunctionCallingFactory.create(LLM_INTERFACE_BEDROCK_CONVERSE_NOVA);
    ...
    @Test
    public void testFactoryCreatesNovaInstance() {
        assertTrue(functionCalling instanceof BedrockConverseNovaFunctionCalling);
    }
    ...
    public void supply() {
        List<LLMMessage> messages = functionCalling
            .supply(List.of(ImmutableMap.of(TOOL_CALL_ID, "test_tool_call_id", TOOL_RESULT, "test result for bedrock converse nova")));
        ...
        Assert.assertEquals("test result for bedrock converse nova", result.getContent().get(0));
    }
