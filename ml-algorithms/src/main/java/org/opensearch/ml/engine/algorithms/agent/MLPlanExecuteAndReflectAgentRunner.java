...
public class MLPlanExecuteAndReflectAgentRunner implements MLAgentRunner {
    ...
    Your role is to receive a discrete Step, run all necessary internal reasoning or tool calls, and return a single, consolidated response that fully addresses that Step.
    If previous context can help you answer the Step, do not call the tool again; instead, reuse whatever information is already available.
    If you are unable to complete the Step or retrieve meaningful information, respond with a clear explanation of what went wrong or what information was missing. Under no circumstances should you end your reply with a question or ask for more information.
    If you search any index, always include the **full raw documents** in your output—do not summarize—so that every piece of retrieved evidence remains visible.
    """;
