        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            lastStepListener.whenComplete(output -> {
                ...
                if (finalI % 2 == 0) {  // LLM response handler to identify next action
                    ...
                    if (memory != null) {
                        memory.save(sessionId, ConversationIndexMessage.conversationIndexMessageBuilder().type("ReAct").question(question).response(thought).finalAnswer(false).sessionId(sessionId).build());
                    }
                    if (finalAnswer != null) {
                        finalAnswer = finalAnswer.trim();
                        if (memory != null) {
                            memory.save(sessionId, ConversationIndexMessage.conversationIndexMessageBuilder().type("ReAct").question(question).response(finalAnswer).finalAnswer(true).sessionId(sessionId).build());
                        }
                    ...
                    if (memory != null) {
                        memory.save(sessionId, ConversationIndexMessage.conversationIndexMessageBuilder().type("ReAct").question(question).response("Action: " + lastAction.get() + "\nAction Input: " + lastActionInput + "\nObservation: " + result).finalAnswer(false).sessionId(sessionId).build());
                    }
                    StringSubstitutor substitutor = new StringSubstitutor(ImmutableMap.of(SCRATCHPAD, scratchpadBuilder.toString()), "${parameters.", "}");
                    newPrompt.set(substitutor.replace(finalPrompt));
                    tmpParameters.put(PROMPT, newPrompt.get());
