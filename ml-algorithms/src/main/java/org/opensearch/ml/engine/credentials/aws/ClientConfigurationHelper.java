...
public class ClientConfigurationHelper {
    ...
    public static ClientConfiguration getConfusedDeputyConfiguration(String[] clusterNameTuple, String region) {
        ...
        LOGGER
            .debug(
                "Adding Source ARN " + domainArn + " and Source Account " + clientId + " in request headers for Confused Deputy Protection"
            );
        return new ClientConfiguration().withHeader(SOURCE_ARN_HEADER, domainArn).withHeader(SOURCE_ACCOUNT_HEADER, clientId);
    }

    protected static String generateDomainArn(String[] clusterNameTuple, String region) {
        String partition = getPartition(region);
        return String.format(OS_DOMAIN_ARN_FORMAT, partition, region, clusterNameTuple[0], clusterNameTuple[1]);
    }
